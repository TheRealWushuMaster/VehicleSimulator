...
from components.motor_curves import MaxPowerVsRPMCurves, PowerEfficiencyCurves
...
def test_motor_curves():
    ...
    max_rpm_point = MotorOperationPoint(power=2_000.0,
                                        rpm=max_rpms)
    ice_power_func = MaxPowerVsRPMCurves.ice(min_rpm=min_rpm_point,
                                            max_rpm=max_rpm_point,
                                            peak_rpm=peak_rpm_point)
    max_eff_point = MotorEfficiencyPoint(power=1_950.0,
                                        rpm=2_000.0,
                                        efficiency=0.35)
    ice_eff_func = PowerEfficiencyCurves.gaussian(max_eff=max_eff_point,
                                                min_eff=min_effi,
    ...
    min_effi = 0.70
    em_power_func = MaxPowerVsRPMCurves.em(base_rpm=1_000.0,
                                        max_rpm=5_000.0,
                                        max_power=2_200.0)
    max_eff_point = MotorEfficiencyPoint(power=1750.0,
                                        rpm=2_000.0,
                                        efficiency=0.95)
    em_eff_func = PowerEfficiencyCurves.gaussian(max_eff=max_eff_point,
                                                min_eff=min_effi,
                                                falloff_rpm=0.0000001,
                                                falloff_power=0.0000002,
